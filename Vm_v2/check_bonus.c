...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/06/05 01:51:33 by ahamouda          #+#    #+#             */
/*   Updated: 2016/06/20 18:44:43 by ahamouda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

static int	is_a_bonus(char *string)
{
...
static void	stock_bonus(t_vm_data *arena, char *string, char *value)
{
	if (string[1] == 'f')
		arena->log_file = 1;
	if (string[1] == 'c')
...
void		check_bonus(int argc, char **argv, t_vm_data *arena)
{
	...
	i = 0;
	while (++i < argc)
		if (argv[i] && is_a_bonus(argv[i]))
			stock_bonus(arena, argv[i], argv[i + 1]);
	if (arena->graph)
	{
		arena->color = 1;
		arena->log_file = 1;
