...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/06/13 14:19:24 by ahamouda          #+#    #+#             */
/*   Updated: 2016/06/14 14:22:30 by ahamouda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		do_and(t_vm_data *arena, t_proc *process)
{
	...
	if (tmp_reg >= 0 && tmp_reg < REG_NUMBER)
	{
		...
		display_and_instruction(process, arena, 0);
	else
		process->carry = 0;
	process->pc = (process->pc + (size + 1) % IDX_MOD) % arena->mem_size;
}
