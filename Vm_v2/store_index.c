...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/06/13 16:16:54 by ahamouda          #+#    #+#             */
/*   Updated: 2016/06/14 15:01:17 by ahamouda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		store_index(t_vm_data *arena, t_proc *process)
{
	...
	tmp_two = get_param_value(arena, process, &size, 2);
	size = 2;
	i = check_reg_number(arena, process, &size);
	if (i)
		tmp_one = get_arg_val(process, arena, tmp_one, 4);
	if (i)
		tmp_two = get_arg_val(process, arena, tmp_two, 2);
	if ((arena->verbosity & 8) == 8)
		display_sti_instruction(process, arena, i);
	if (i && (arena->verbosity & 8) == 8)
		ft_printf_fd(arena->fd, "It did stock r%d at (%d + %d) bytes away\n",
			tmp_reg, tmp_two, tmp_one);
	if (i)
	{
		arena->field[(process->pc + tmp_one + tmp_two) % arena->mem_size] =
			(process->reg[tmp_reg - 1] >> 24) & 0xFF;
		arena->field[(process->pc + tmp_one + tmp_two + 1) % arena->mem_size] =
			(process->reg[tmp_reg - 1] >> 16) & 0xFF;
		arena->field[(process->pc + tmp_one + tmp_two + 2) % arena->mem_size] =
			(process->reg[tmp_reg - 1] >> 8) & 0xFF;
		arena->field[(process->pc + tmp_one + tmp_two + 3) % arena->mem_size] =
