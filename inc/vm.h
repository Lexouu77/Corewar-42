...
/*   Updated: 2016/09/12 06:13:28 by adjivas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long			get_n_param_value(t_vm_data *arena, t_proc *proc, int nb,
				int *err);
int				get_n_param_size(t_vm_data *arena, int n);
int				get_n_reg_param_value(t_vm_data *arena, t_proc *process,
				int nb,
				int *error);
...
int				stock_fixed_parameter_value(t_vm_data *arena,
				t_instr *instruction,
...
void			fill_field(t_vm_data *arena, t_player *player, int prog_sz,
				int fd);
t_vm_data		*init_data(void);
int				init_game(char **argv, t_vm_data *arena, int argc);
void			init_process(t_vm_data *arena);
void			set_color_field_owner(t_vm_data *arena, int size,
				t_proc *process,
				int index);
...
void			stock_players_and_number(char **argv, t_vm_data *arena,
				int argc);
